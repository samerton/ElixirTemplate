...
<html lang="en">
  <head>
    ...
    <?php $title = $donate_language['donate']; ?>
      ...
      if($settings[2]->value == 0 && !$user->isLoggedIn()){
          ...
          if($settings[3]->value == 1){
              ...
              foreach($latest_donors as $latest_donor){
                  ...
                  $latest_donors_string .= '<p><div class="row vertical-align"><div class="col-md-3" style="padding-right:0;"><a href="/profile/' . htmlspecialchars($latest_donor->ign) . '"><img class="img-circle" src="https://cravatar.eu/avatar/' . ((strlen($latest_donor->uuid) > 0 && $latest_donor->uuid != '----') ? htmlspecialchars($latest_donor->uuid) : htmlspecialchars($latest_donor->ign)) . '/30.png" /></a></div>';
                  $latest_donors_string .= '<div class="col-md-9" style="padding-left:0px;"><a href="/profile/' . htmlspecialchars($latest_donor->ign) . '">' . htmlspecialchars($latest_donor->ign) . '</a> - ' . $currency . $latest_donor->price . '<br />' . date('d M Y', $latest_donor->time) . '</div></div></p>';
              }
      
              $smarty->assign('LATEST_DONORS', $donate_language['latest_donors']);
              $smarty->assign('LATEST_DONORS_LIST', $latest_donors_string);
              $smarty->assign('STORE', $donate_language['donate']);
              
              // Display categories
              $categories = $queries->orderAll('donation_categories', '`order`', 'ASC');
      
              $categories_list = '<ul class="nav nav-pills">';
              ...
              foreach($categories as $category){
                  ...
                  else if (count($packages) == 5) {
                      $col = '4';
                  }
                  else if (count($packages) == 4) {
                      $col = '6';
                  }
                  else if (count($packages) == 3) {
                      $col = '4';
                  }
                  else if (count($packages) == 2) {
                      $col = '6';
                  }
                    ...
                    $categories_content .= '<div class="col-md-' . $col . '">
      										  <div class="panel-body">
      										    <h5 style="font-size:14px;">
      											  ...
      											  <span class="pull-right">' . (($package->cost > 0) ? $currency . htmlspecialchars($package->cost) : '') . '</span>
      											</h5>
      											<hr />
