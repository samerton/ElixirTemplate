    ...
    <?php
      ...
      if($forum_layout == '1'){
      	...
      	while($n < $limit){
      		...
      		$last_reply_avatar = '<img class="img-centre img-circle" style="max-height:30px;max-width:30px;" src="' .  $user->getAvatar($discussions[$n]['topic_last_user'], "../", 30) . '" />';
      		...
      		foreach($forums['forums'] as $item){
      			if($item['parent'] == $parent['id']){
                    ...
                    // Count posts
      				...
      				$last_reply_avatar = '<img class="img-centre img-circle" style="max-height:30px;max-width:30px;" src="' .  $user->getAvatar($item['last_user_posted'], "../", 30) . '" />';
      	...
      	foreach($latest as $item){
      		if($n >= 5){ break; }
      		// Get avatar of user
      		$last_reply_avatar = '<img class="img-centre img-circle" style="max-height:30px;max-width:30px;" src="' .  $user->getAvatar($item['topic_last_user'], "../", 30) . '" />';
