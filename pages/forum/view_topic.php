...
<html lang="en">
  <head>
	...
	<?php $title = htmlspecialchars($topic->topic_title); ?>
  ...
  <body>
    ...
    <?php
      ...
      while($n < $d){
      	...
      	$avatar = '<img class="img-circle" style="width:100px; height:100px;" src="' . $user->getAvatar($posts[$n]->post_creator, '../', 100) . '" />';
      	...
      	if($user->isLoggedIn()) { 
      		...
      		if($user->canViewMCP($user->data()->id)){ // Admins and moderators
      			$buttons .= '<a rel="tooltip" title="' . $forum_language['edit_post'] . '" href="/forum/edit_post/?pid=' . $posts[$n]->id . '&amp;tid=' . $tid . '" class="btn btn-default btn-sm"><span class="fas fa-edit"></span></a>';
      		...
      		if($user->canViewMCP($user->data()->id)){ // Mods/admins only
      			...
      			<form onsubmit="return confirm(\'' . $forum_language['confirm_post_deletion'] . '\');" style="display: inline;" action="/forum/delete_post/" method="post">
      				...
      				<button rel="tooltip" title="' . $forum_language['delete_post'] . '" type="submit" class="btn btn-danger btn-sm">
      		...
      		// Report button
      		$buttons .= '<a rel="tooltip" title="' . $forum_language['report_post'] . '" href="/forum/report_post/?pid=' . $posts[$n]->id . '&amp;tid=' . $tid . '" class="btn btn-warning btn-sm"><span class="fas fa-exclamation"></span></a>';
      
      		// Quote button
      		if($can_reply){
      			if($user->canViewMCP($user->data()->id)){ 
      				$buttons .= ' <a rel="tooltip" title="' . $forum_language['quote_post'] . '" href="/forum/create_post/?tid=' . $tid . '&amp;qid=' . $posts[$n]->id . '&amp;fid=' . $topic->forum_id . '" class="btn btn-info btn-sm"><span class="fas fa-quote-left"></span></a>';
      			} else { 
      				if($topic->locked != 1){ 
      					$buttons .= ' <a rel="tooltip" title="' . $forum_language['quote_post'] . '" href="/forum/create_post/?tid=' . $tid . '&amp;qid=' . $posts[$n]->id . '&amp;fid=' . $topic->forum_id . '" class="btn btn-info btn-sm"><span class="fas fa-quote-left"></span></a>';
      				}
      	    ...
      	    if(count($reputation)){
      			...
      			if($user_has_given === false){
      				...
      				<form style="display: inline;" action="/forum/reputation/" method="post">
      					...
      					<button rel="tooltip" title="' . $forum_language['give_reputation'] . '" type="submit" class="btn btn-success btn-sm give-rep"><span class="far fa-thumbs-up"></span></button>
      				</form>';
      			} else {
      				$post_reputation .= '
      				<form style="display: inline;" action="/forum/reputation/" method="post">
      					...
      					<button rel="tooltip" title="' . $forum_language['remove_reputation'] . '" type="submit" class="btn btn-danger btn-sm give-rep"><span class="far fa-thumbs-down"></span></button>
      				</form>';
      			}
      		  ...
      		  <div class="modal-content">
      			...
      			if(count($reputation)){
      				...
      				foreach($reputation as $rep){
      					$post_reputation .= '
      					  <tr>
      						<td style="width:40px"><a href="/profile/' . htmlspecialchars($user->IdToMCName($rep->user_given)) . '"><img class="img-circle" style="height:30px; width:30px;" src="' . $user->getAvatar($rep->user_given, '../', 30) . '" /></a></td>
