...
<html lang="en">
  <head>
    ...
    <?php $title = $navbar_language['forum'] . ' - ' . $forum_language['edit_post']; ?>
    <?php require('core/includes/template/generate.php'); ?>
  </head>
  
  <body>
    <?php $smarty->display('styles/templates/' . $template . '/navbar.tpl'); ?>
    <?php if(Session::exists('failure_post')){ $smarty->assign('SESSION', Session::flash('failure_post')); } else { $smarty->assign('SESSION', ''); } ?>
    <?php  
      $smarty->assign('EDITING_POST', $forum_language['editing_post']);
      
      if(isset($edit_title)){
      	$edit_title_form = '<div class="form-group"><input type="text" class="form-control" name="title" value="' .  $post_title . '"></div>';
      	...
      	if(count($forum_labels)){
      		...
      		$edit_labels_form = '<div class="panel-body" style="padding: 10px 15px;"><strong>' . $forum_language['label'] . ':</strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp<input type="radio" name="topic_label" value="0"';
      		
      		if($post_label == 0 || $post_label == null){
      			$edit_labels_form .= ' checked';
      		}
      		
      		$edit_labels_form .= '> <span class="label label-default">' . $general_language['none'] . '</span>&nbsp;&nbsp;<span class="inline">';
    ...
    <script type="text/javascript">
      ...
      CKEDITOR.config.enterMode = CKEDITOR.ENTER_BR;
         <?php if(!Session::exists('failure_post')){ ?>
