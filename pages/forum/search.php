...
<html lang="en">
  <head>
	...
	<?php $title = $navbar_language['forum']; ?>
  ...
  <body>
	...
	<div class="container index">
      <h3><?php echo $general_language['search']; ?></h3>
      <br />
	  <?php if($user->isLoggedIn()){ if(Session::exists('forum_search')){ echo Session::flash('forum_search'); }
      ...
      <div class="panel panel-primary">
        <div class="panel-heading">
          <a href="/forum/view_topic/?tid=<?php echo $merged_results[$n]['topic']['topic_id']; ?>&amp;pid=<?php echo $merged_results[$n]['post']['post_id']; ?>"><?php echo htmlspecialchars($merged_results[$n]['topic']['topic_title']); ?></a>
        </div>
        <div class="panel-body">
          <div><?php echo $purifier->purify(htmlspecialchars_decode($merged_results[$n]['post']['post_content'])); ?></div>
          <hr />
          <?php if(!($merged_results[$n]['topic']['topic_title']) && $user->canViewMCP($user->data()->id)){ ?>
		    ...
		    <a href="#" onclick="document.getElementById('form<?php echo $n; ?>').submit()" class="btn btn-sm btn-danger">Delete</a>
		  <?php } ?>
          <span class="post-meta">By <a href="/profile/<?php echo htmlspecialchars($user->idToMCName($merged_results[$n]['post']['post_creator'])); ?>"><?php echo htmlspecialchars($user->idToName($merged_results[$n]['post']['post_creator'])); ?></a> : <?php echo date('d M Y, H:i', strtotime($merged_results[$n]['post']['post_date'])); ?></span>
          ...
          <div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
    	    <form class="form-horizontal" role="form" method="post" action="">
    	      <div class="form-group">
                <label class="field-name">Search</label>
    	        <input type="text" class="form-control" name="forum_search">
