...
<html lang="en">
  <head>
    ...
    <?php if(isset($custom_meta)){ echo $custom_meta; } ?>
	<?php $title = htmlspecialchars($forum_query->forum_title); ?>
  ...
  <body>
    ...
    <?php if(Session::exists('success_post')){ echo '<div class="container"><center>' . Session::flash('success_post') . '</center></div>'; } ?>
      ...
      if($forum_layout == '1'){
      	if(!count($stickies) && !count($topics)){
      		...
      		if($user->isLoggedIn() && $forum->canPostTopic($fid, $user->data()->group_id)){ 
      			$smarty->assign('NEW_TOPIC_BUTTON', '<a style="float: right;" href="/forum/new_topic/?fid=' . $fid . '" class="btn btn-primary">' . $forum_language['new_topic'] . '</a>');
      		} else {
      		...
      		foreach($stickies as $sticky){
      			...
      			$last_reply_avatar = '<img class="img-circle img-centre" style="max-height:30px;max-width:30px;"  src="' . $user->getAvatar($sticky->topic_last_user, "../", 30) . '" />';
      		...
      		while($n < $d){
      			...
      			// Get a string containing HTML code for a user's avatar. This depends on whether custom avatars are enabled or not, and also which Minecraft avatar source we're using
      			$last_reply_avatar = '<img class="img-circle img-centre" style="max-height:30px;max-width:30px;"  src="' . $user->getAvatar($topics[$n]->topic_last_user, "../", 30) . '" />';
      	...
      	if(count($subforums)){
      		...
      		foreach($subforums as $subforum){
      			if($forum->forumExist($subforum->id, $user->data()->group_id)){
      				...
      				$last_reply_avatar = '<img class="img-circle img-centre" style="max-height:30px;max-width:30px;"  src="' . $user->getAvatar($subforum->last_user_posted, "../", 30) . '" />';
      	...
      	if(!count($stickies) && !count($topics)){
      		...
      		foreach($latest as $item){
      			...
      			// Get avatar of user
      			$last_reply_avatar = '<img class="img-circle img-centre" style="max-height:30px;max-width:30px;"  src="' . $user->getAvatar($item['topic_last_user'], "../", 30) . '" />';
